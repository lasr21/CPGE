...
const state = { data: null, unit: "nominal", tab: "principal", loadToken: 0, manifest: null, selectedYear: null };
...
function valueLabel(node) { if (state.unit === "pct") return `${money(valueFor(node), 1)}% del PIB`; if (state.unit === "percapita") return `$${money(valueFor(node), 0)} por persona`; return mmp(valueFor(node)); }
...
function renderSummary(displayYear = state.data.meta.anio_fiscal) {
  ...
  const series = state.manifest?.anios.find((item) => item.archivo === state.selectedYear);
  const warning = series?.nota ?? data.meta.advertencias?.[0] ?? "Cifras del paquete económico.";
  $("#warnings").innerHTML = `<p><strong>${data.meta.tipo_documento} ${displayYear}:</strong> ${warning} <a href="${data.meta.url_fuente}" target="_blank" rel="noreferrer">Ver fuente oficial de Hacienda</a></p>`;
  $("#data-source").innerHTML = `<strong>Fuente y estado del dato</strong><br>${data.meta.fuente_documento}. ${series?.nota ?? "Este archivo conserva las cifras publicadas en el documento."} <a href="${data.meta.url_fuente}" target="_blank" rel="noreferrer">Consultar Paquete Económico en Hacienda</a>`;
}
...
function renderSankey() {
  ...
  addTree(data.gasto);
  const addIncomeTree = (node, parent = null, depth = 0) => {
    if (depth > 2) return;
    nodes.push({ name: node.id, title: node.nombre, color: "ingreso" });
    if (parent) links.push({ source: parent.id, target: node.id, value: node.monto_mdp });
    if (children(node).length) children(node).forEach((child) => addIncomeTree(child, node, depth + 1));
    else links.push({ source: node.id, target: data.gasto.id, value: node.monto_mdp });
  };
  addIncomeTree(data.ingresos);
  nodes.push({ name: "deuda_anio", title: "Deuda del año", color: "deuda" });
  links.push({ source: "deuda_anio", target: data.gasto.id, value: debt });
...
async function load() { const manifest = await fetch(`${ROOT}/manifest.json`).then((response) => response.json()); state.manifest = manifest; const ready = manifest.anios.filter((year) => year.estado !== "pendiente"); const select = $("#year-select"); select.innerHTML = ready.map((year) => `<option value="${year.archivo}">${year.etiqueta}</option>`).join(""); const defaultYear = ready.find((year) => year.anio === manifest.anio_default) ?? ready[0]; select.value = defaultYear.archivo; select.addEventListener("change", () => loadYear(select.value)); await loadYear(defaultYear.archivo); }
async function loadYear(file) { const token = ++state.loadToken; state.selectedYear = file; const data = await fetch(`${ROOT}/${file}`).then((response) => response.json()); if (token !== state.loadToken) return; state.data = data; renderSummary(Number(file.match(/\d{4}/)?.[0]) || data.meta.anio_fiscal); renderSankey(); renderDestinations(); renderTable(); }
